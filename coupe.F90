PROGRAM coupe
  ...
  !
  PRINT *,' '
  PRINT *, 'Coupe version  F90 24/11/2010'
  ...
  DO jt = 1, max_tstp   ! loop on time step
     ...
     IF (ntime(jt) == 1) THEN
        ...
        IF (opt_clrdata == 1) THEN
           ...
           blimgclr%lclr = .TRUE. 
           blimgclr%lcnt = .FALSE.
           CALL CalculateCutPlane (clrdata, blimgclr, blimgzlevel, jt, ncl_dim)
        ENDIF
        ...
        IF (opt_contours == 1) THEN
           ...
           blimgcnt%lcnt = .TRUE. 
           blimgcnt%lclr = .FALSE.
           CALL CalculateCutPlane (cntdata, blimgcnt, blimgzlevel, jt, nct_dim)
